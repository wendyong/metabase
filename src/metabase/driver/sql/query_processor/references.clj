(ns metabase.driver.sql.query-processor.references
  ...
  (merge
   ...
   (when (and joins join-alias)
     (when-let [matching-join (some
                               ...
                               joins)]
       (merge
        (when-let [info (extra-field-info driver matching-join (mbql.u/update-field-options field-clause dissoc :join-alias))]
          (println "info:" (pr-str info)) ; NOCOMMIT
          info)
