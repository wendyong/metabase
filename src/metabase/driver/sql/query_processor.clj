(ns metabase.driver.sql.query-processor
  ...
  (:require [clojure.core.match :refer [match]]
            ...
            [metabase.driver.sql.query-processor.alias :as sql.qp.alias]
            [metabase.driver.sql.query-processor.merged-select :as sql.qp.merged-select]
            [metabase.driver.sql.query-processor.references :as sql.qp.references]
            [metabase.driver.sql.query-processor.subselect :as sql.qp.subselect]
...
(comment sql.qp.alias/keep-me)

;; these multimethods used to live here.
(p/import-vars
 [sql.qp.references source-query-alias]
 [sql.qp.alias prefix-field-alias field->alias])
...
(p.types/defrecord+ SQLSourceQuery [sql params]
  hformat/ToSql
  (to-sql [_]
...
(defmethod ->honeysql [:sql (class Field)]
  ...
  (as-> (let [{schema :schema, table-name :name} (qp.store/table table-id)]
...
(defn field-reference-info [field-clause]
  (get (:sql.qp/references *query*) field-clause))

(defmethod ->honeysql [:sql :field]
  [driver [_ field-id-or-name options :as field-clause]]
  (when (= field-id-or-name 127617)
    (println "REFERENCES:\n" (u/pprint-to-str 'yellow (:sql.qp/references *query*)))              ; NOCOMMIT
    #_(println "select-info:" (pr-str select-info))                                                 ; NOCOMMIT
    (println "(field-reference-info field-clause):" (pr-str (field-reference-info field-clause))) ; NOCOMMIT
    )
  (binding [*field-options* options]
    (let [{:keys [source-table source-alias]} (field-reference-info field-clause)
          field-id-or-name                    (or source-alias field-id-or-name)
          honeysql-form                       (if (integer? field-id-or-name)
                                                (->honeysql driver (qp.store/field field-id-or-name))
                                                (cond-> (->honeysql driver (hx/identifier :field source-table field-id-or-name))
...
(defmethod ->honeysql [:sql :avg]        [driver [_ expr]]   (hsql/call :avg             (->honeysql driver expr)))
(defmethod ->honeysql [:sql :median]     [driver [_ expr]]   (hsql/call :median          (->honeysql driver expr)))
(defmethod ->honeysql [:sql :percentile] [driver [_ expr p]] (hsql/call :percentile-cont (->honeysql driver expr) (->honeysql driver p)))
(defmethod ->honeysql [:sql :distinct]   [driver [_ expr]]   (hsql/call :distinct-count  (->honeysql driver expr)))
(defmethod ->honeysql [:sql :stddev]     [driver [_ expr]]   (hsql/call :stddev_pop      (->honeysql driver expr)))
(defmethod ->honeysql [:sql :var]        [driver [_ expr]]   (hsql/call :var_pop         (->honeysql driver expr)))
(defmethod ->honeysql [:sql :sum]        [driver [_ expr]]   (hsql/call :sum             (->honeysql driver expr)))
(defmethod ->honeysql [:sql :min]        [driver [_ expr]]   (hsql/call :min             (->honeysql driver expr)))
(defmethod ->honeysql [:sql :max]        [driver [_ expr]]   (hsql/call :max             (->honeysql driver expr)))

;; order-by should be a HoneySQL map like {:order-by [...]}
(p.types/defrecord+ CumulativeSum [expr]
  hformat/ToSql
  (to-sql [_]
    (format "sum(%s) OVER (ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)"
            (hformat/to-sql expr))))

(defmethod ->honeysql [:sql :cum-sum]
  [driver [_ expr]]
  (CumulativeSum. (->honeysql driver expr)))

(defmethod ->honeysql [:sql :cum-sum]
  [driver [_ expr]]
  (->honeysql [:sql [:sum expr]]))

(defmethod ->honeysql [:sql :floor] [driver [_ expr]] (hsql/call :floor (->honeysql driver expr)))
(defmethod ->honeysql [:sql :ceil]  [driver [_ expr]] (hsql/call :ceil  (->honeysql driver expr)))
(defmethod ->honeysql [:sql :round] [driver [_ expr]] (hsql/call :round (->honeysql driver expr)))
(defmethod ->honeysql [:sql :abs]   [driver [_ expr]] (hsql/call :abs (->honeysql driver expr)))

(defmethod ->honeysql [:sql :log]   [driver [_ expr]] (hsql/call :log 10 (->honeysql driver expr)))
(defmethod ->honeysql [:sql :exp]   [driver [_ expr]] (hsql/call :exp (->honeysql driver expr)))
(defmethod ->honeysql [:sql :sqrt]  [driver [_ expr]] (hsql/call :sqrt (->honeysql driver expr)))

(defmethod ->honeysql [:sql :power]
  [driver [_ expr power]]
  (hsql/call :power (->honeysql driver expr) (->honeysql driver power)))
...
(defmethod apply-top-level-clause [:sql :breakout]
  [driver _ honeysql-form {breakout-fields :breakout, :as query}]
  (apply h/group honeysql-form (mapv (partial ->honeysql driver) breakout-fields)))

(defmethod apply-top-level-clause [:sql :sql.qp/select]
  [driver _ honeysql-form {fields :sql.qp/select, :as query}]
  (apply h/merge-select honeysql-form (vec (for [{:keys [clause alias], :as info} fields]
                                             [(->honeysql driver clause) (hx/identifier :field-alias alias)]))))
...
(s/defmethod join->honeysql :sql :- HoneySQLJoin
  ...
  [[(join-source driver join)
   ...
   (binding [*query* (update *query* :sql.qp/references merge (:sql.qp/references join))]
...
(ns-unmap *ns* 'add-default-select)
#_(defn- add-default-select
...
(defn- apply-clauses
  ...
  (binding [*query* inner-query]
    #_(println "QUERY IS\n" (u/pprint-to-str 'red *query*)) ; NOCOMMIT
    (if source-query
...
(defn- preprocess-query [driver inner-query]
  (u/prog1 (->> inner-query
                (sql.qp.merged-select/add-merged-select driver)
                (sql.qp.references/add-references driver)
                sql.qp.subselect/subselectify)
    (when-not (= inner-query <>)
...
(defn mbql->honeysql
  ...
  (u/prog1 (apply-clauses driver {} (preprocess-query driver inner-query))
    #_(println (format "\nHoneySQL Form: %s\n%s" (u/emoji "🍯") (u/pprint-to-str 'cyan <>))) ; NOCOMMIT
